...
Requires a non-empty thread. Refuses with the busy message when `inFlight || widgetStatus` and holds the `inFlight` claim for its own duration (reset in `finally`), so a second summarize — or a `/btw` fired while it runs — cannot double-deliver or have its thread reset out from under it. Makes a single tool-less model call — `ctx.modelRegistry.streamSimple(settings.model, { systemPrompt: BTW_SUMMARIZE_SYSTEM_PROMPT, messages: [{ role: "user", content: buildSummarizePrompt(pendingThread) }] }, { signal: AbortSignal.timeout(SUMMARIZE_TIMEOUT_MS) })` — awaits `.result()`, extracts and sanitizes the text answer, wraps in `<btw-summary>` tags with optional instructions, delivers exactly like inject, then resets. No reasoning is passed (the summary runs with thinking off). The call is bounded by `AbortSignal.timeout(SUMMARIZE_TIMEOUT_MS)` (120 s) instead of the main btw thread's `AgentSession`: streamSimple resolves auth at request time (so a btw-only model override needs only the resolved `Model`, no pre-fetched credentials) and honors a real signal, so there is no loader, session, subscribe, dispose, or abort timer to manage. A synchronous auth throw, an in-stream `stopReason` of `"error"`/`"aborted"` (a fired timeout arrives as `"aborted"`), or an empty answer all land in the same `catch`: error notify, nothing delivered, thread + widget preserved for retry. The extraction seam is a pure `callModel(ctx, model, context, options) => { text, stopReason, errorMessage }` helper, which tests drive by stubbing `ctx.modelRegistry.streamSimple`. Only this path migrated off `createAgentSession`; the main multi-turn `/btw` thread (`runBtw`/`ensureSession`) is a seeded conversation with read-only tools and stays an `AgentSession`.

**`/btw:model [<provider> <model> <api> | clear]`**
No args → show resolved model and source (`override` / `inherits main thread` / fallback reason). `clear` → drop override, persist `btw-model-override {action:"clear"}`. Three tokens → resolve via `ctx.modelRegistry.find(provider, id)`; unknown → error notify ("use /login or /models first"). On set: persist `btw-model-override {action:"set", provider, id, api}`, **dispose the sub-session but keep `pendingThread`** — the next question builds a fresh sub-session reseeded from the preserved hidden thread (dbachelder semantics). Disposing mid-answer aborts an in-flight sub-session, so immediately after `disposeSession()` any still-streaming slot is dropped (`cancelInFlightSlots`) with a "btw: in-flight answer cancelled" notify — finished exchanges are kept. At question time, if the override has no credentials (`ctx.modelRegistry.getApiKeyAndHeaders`), fall back to `ctx.model` with a warning notify.
...
\
A: …", display: true, details})` **does** participate in context, so register `pi.on("context", e => ({ messages: e.messages.filter(m => m.customType !== "btw-note") }))` and a `pi.registerMessageRenderer("btw-note", …)` for display (dbachelder pattern). The note is always sent with `{ deliverAs: "followUp", triggerTurn: false }` (see `buildSaveNoteDelivery`), **not** a bare `{ deliverAs: "followUp" }`: while the main agent is streaming, `deliverAs: "followUp"` alone reaches the `agent.followUp` branch of `sendCustomMessage`, which the loop drains after the turn with a fresh model call — and the context hook then strips the note, so the main model is invoked with nothing new to say. `triggerTurn: false` instead lands the note in `_pendingCustomMessages`, appended after the turn with no model call (appended directly when idle). Notify "saved" when idle, "queued" when busy.
- Notes must be filtered out of the contextual seed on the **pre-conversion** `AgentMessage[]`, because `convertToLlm` maps role `"custom"` to a plain user message and drops `customType` — a filter run after conversion never matches. See `ensureSession`; `sanitizeMainMessages` only strips the trailing in-progress assistant now.
...
│ …last 3 thinking lines in dim italic ▍                    ← while streaming with no answer yet: only the tail, cursor on it
│ 💭 thought 42 lines                                       ← once done: thinking collapses to a one-line marker
...
Rules: border width fixed at 54 columns (noahsaso's proportions); at most the **last 3** exchanges are rendered in full — older ones are represented only by the header count; the streaming slot is always shown; error slots render the message with `✗` in error color; the `▍` cursor appears on the thinking tail while no answer text exists yet, else on the answer tail, and disappears when `done`. Thinking is never rendered in full: while streaming with no answer yet only the last ~3 lines show; once the answer streams thinking disappears; a finished slot collapses it to a one-line `💭 thought N lines` marker (a reasoning model's full thinking block would otherwise stack dozens of wrapped lines above the editor per exchange). Persisted `btw-exchange.thinking` is likewise capped (500 chars) — no consumer reads it. Empty `slots` → widget cleared entirely.
